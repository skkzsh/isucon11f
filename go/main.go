...
func (h *handlers) GetGrades(c echo.Context) error { // FIXME: 高速化
	...
	registeredCoursesQuery := "SELECT `courses`.*" +
		" FROM `registrations`" +
		" JOIN `courses` ON `registrations`.`course_id` = `courses`.`id`" +
		" WHERE `user_id` = ?"
	if err := h.DB.Select(&registeredCourses, registeredCoursesQuery, userID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
	...
	if err := h.DB.Select(&registeredCoursesClasses, registeredCoursesClassesQuery, params...); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func calcGpas(db *sqlx.DB) (error, []float64) {
	...
	query := "SELECT IFNULL(SUM(`submissions`.`score` * `courses`.`credit`), 0) / 100 / `credits`.`credits` AS `gpa`" +
		...
		" WHERE `users`.`type` = ?" +
		" GROUP BY `users`.`id`"
	err := db.Select(&gpas, query, StatusClosed, StatusClosed, Student)
	return err, gpas
}
