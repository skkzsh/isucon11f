...
func (h *handlers) GetGrades(c echo.Context) error {
	...
	if err := h.DB.Select(&UserCourseTotalList, query); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
	UserCourseTotalMap := make(map[string][]int, len(UserCourseTotalList))
	for _, t := range UserCourseTotalList {
		UserCourseTotalMap[t.CourseID] = append(UserCourseTotalMap[t.CourseID], t.Total)
	}
	...
	for _, course := range registeredCourses {
		...
		// 	return c.NoContent(http.StatusInternalServerError)
		// }
		totals = UserCourseTotalMap[course.ID]
